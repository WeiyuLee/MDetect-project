...
class DETECTION_MODEL(object):
    ...
    def get_batch_data_idx(self, num, batch_size):
        ...
        random.shuffle(idx)
        batch_data_idx = idx[:batch_size//2]
    ...
    def build_baseline(self):###
        ...
        self.label_flatten = tf.reshape(self.label, [tf.shape(self.input)[0], -1])
        ...
        self.l2_loss = tf.pow(self.pred - self.label_flatten, 2)
        ...
        with tf.name_scope('train_summary'):
            ...
            tf.summary.image("code_image", self.label, collections=['train'])
            tf.summary.image("output_image", self.pred_reshape, collections=['train'])
        ...
        with tf.name_scope('test_summary'):
            ...
            tf.summary.image("code_image", self.label, collections=['test'])
            tf.summary.image("output_image", self.pred_reshape, collections=['test'])
    ...
    def train_baseline(self):
        ...
        print("Test normal sample: {}".format(len(test_n_image)))
        print("Train normal sample: {}".format(len(train_n_image)))
        print("Test abnormal sample: {}".format(len(test_ab_image)))
        print("Train abnormal sample: {}".format(len(train_ab_image)))        
        ...
        for it in iter_pbar:            
            ...
            n_idx = self.get_batch_data_idx(len(train_n_image), self.batch_size)
            ab_idx = self.get_batch_data_idx(len(train_ab_image), self.batch_size)
